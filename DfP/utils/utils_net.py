...
class ImgFeatureExtractorModule(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        ...
        # self.proj = MetaConv2d(in_c, embed_dim, kernel_size=3, stride=1, padding=1, bias=bias)
...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.body = MetaSequential(MetaConv2d(n_feat, n_feat//2, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.PixelUnshuffle(2))
...
class FeatureTransform(nn.Module):
    def __init__(self, dim):
        ...
        self.conv1 = MetaSequential(MetaConv2d(dim, dim*2, kernel_size=3, stride=1, padding=1, bias=False),
                                    LayerNorm(dim*2),
                                  nn.SiLU())
        self.conv2 = MetaSequential(MetaConv2d(dim*2, dim*2, kernel_size=3, stride=1, padding=1, bias=False),
                                    LayerNorm(dim*2),
                                  nn.SiLU())
        self.conv3 = MetaConv2d(dim*2, dim, kernel_size=3, stride=1, padding=1, bias=False)
    def forward(self, x, meta=False):
...
class FT(nn.Module):
    ...
    def forward(self, demfeat, meta=False):
        TF = {}    
        TF['TF1'] = F.interpolate(self.FT1(demfeat['DF1'], meta=meta), scale_factor=2, mode='bilinear')
        TF['TF2'] = F.interpolate(self.FT2(demfeat['DF2'], meta=meta), scale_factor=2, mode='bilinear')
        TF['TF3'] = F.interpolate(self.FT3(demfeat['DF3'], meta=meta), scale_factor=2, mode='bilinear')

        return TF
    
class MetaFeatureGenerator(nn.Module):
    def __init__(self, dim,input_dim=96):
        super(MetaFeatureGenerator, self).__init__()

        # self.conv_dem = MetaSequential(MetaConv2d(dim, dim*4, kernel_size=3, stride=1, padding=1, bias=False),
        #                                LayerNorm(dim*4),
        #                                nn.SiLU(),
        #                           nn.PixelShuffle(2),
        ...
        #                           LayerNorm(dim*2),
        #                           nn.SiLU(),
        #                           MetaConv2d(dim*2, dim*2, kernel_size=3, stride=1, padding=1, bias=False))

        # self.conv_task = MetaSequential(MetaConv2d(dim, dim*2, kernel_size=3, stride=1, padding=1, bias=False),
        #                                 LayerNorm(dim*2),
        #                                  nn.SiLU(),
        #                                  MetaConv2d(dim*2, dim, kernel_size=3, stride=1, padding=1, bias=False))
        # 改动
        self.conv_task = MetaSequential(
            MetaConv2d(input_dim, dim, kernel_size=3, stride=1, padding=1, bias=False),
            MetaConv2d(dim, dim*2, kernel_size=3, stride=1, padding=1, bias=False),
                                        LayerNorm(dim*2),
                                         nn.SiLU(),
                                         MetaConv2d(dim*2, dim*2, kernel_size=3, stride=1, padding=1, bias=False),
                                            ...
                                            nn.SiLU(),
                                         MetaConv2d(dim*2, dim, kernel_size=3, stride=1, padding=1, bias=False))
        # self.conv_fuse = MetaSequential(MetaConv2d(dim*4, dim*8, kernel_size=3, stride=1, padding=1, bias=False),
        #                                 LayerNorm(dim*8),
        #                           nn.SiLU(),
        #                           MetaConv2d(dim*8, dim*8, kernel_size=3, stride=1, padding=1, bias=False),
        #                             LayerNorm(dim*8),
        #                           nn.SiLU(),
        #                           MetaConv2d(dim*8, dim*4, kernel_size=3, stride=1, padding=1, bias=False),
        #                             LayerNorm(dim*4),
        #                           nn.SiLU(),
        #                           MetaConv2d(dim*4, dim*2, kernel_size=3, stride=1, padding=1, bias=False),
        #                             LayerNorm(dim*2),
        #                           nn.SiLU(),
        #                           MetaConv2d(dim*2, dim, kernel_size=3, stride=1, padding=1, bias=False))
    ...
    def forward(self, xd, xt, meta=False):
        # xd = self.conv_dem(xd,meta=meta)
        xt = self.conv_task(xt,meta=meta)
        # x = torch.cat([xd, xt], 1)
        # x = self.conv_fuse(x,meta=meta)
        return xt
...
class MFG(nn.Module):
    def __init__(self, dim=48):
        ...
        # 改动
        self.MFG1 = MetaFeatureGenerator(dim*2,input_dim=96)
        self.MFG2 = MetaFeatureGenerator(dim*2,input_dim=96)
        self.MFG3 = MetaFeatureGenerator(dim*4,input_dim=192)
...
class PIDNet(nn.Module):
    def __init__(self, inp_channels=12, out_channels=12, dim=48, num_blocks=[4, 4, 4], heads=[1, 2, 4], ffn_expansion_factor=2.66, bias=False):
    ...
    def forward(self, inp_img, ELT_state=True, meta=False):
        ...
        x3_up = self.reduce_chan3(torch.cat([x3_up, x2], 1),meta=meta)       # (B, 2C, H/2, W/2)
        ...
        imgout = self.output(self.refinement(self.upsample(x1_dec,meta=meta),meta=meta),meta=meta) + F.interpolate(inp_img, scale_factor=2, mode='bilinear')
        imgout = torch.clamp(imgout, 0, 1)
        if ELT_state:
            return imgout
        else:
            demfeat = {}
...
class PolFreeNet(nn.Module):
    def __init__(self, inp_channels=24, out_channels=3, dim=48, num_blocks=[4, 4, 4], heads=[1, 2, 4], ffn_expansion_factor=2.66, bias=False):
        super(PolFreeNet, self).__init__()
