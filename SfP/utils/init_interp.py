...
def get_CPFA(img, pattern):
    ...
    if len(pr) + len(pg) + len(pb) != 4:
        raise ValueError(
            "Invalid mosaic pattern. It must contain exactly 4 characters."
        )
...
def deomsaicking(img_lr, polar_angles=[90, 45, 0, 135]):
    ...
    for i in range(batch_size):
        ...
        for j in range(num_polar_angles):
            ...
            rgb_img_current = (
                img_lr[i, start_channel:end_channel].permute(1, 2, 0).cpu().numpy()
            )
        ...
        final_resized_image = np.concatenate(
            resized_images, axis=-1
        )  # Shape: [h*2, w*2, len(polar_angles)*3]
        decoded_tensor = (
            torch.from_numpy(final_resized_image).permute(2, 0, 1).float() / 255.0
        )
    ...
    return final_result.to(img_lr.device)
...
class init_interp(nn.Module):
    def __init__(self, pattern="rggb", phase="train"):
        super().__init__()
