...
- `polarapp/evaluation.py` contains prediction, evaluation, and output routines.
...
TaskNet output at twice the input height and width.
...
Evaluation additionally requires matching `normal` and `mask` folders. It uses
the simulated-imaging evaluation path. The saved surface normal has the same
height and width as the task ground truth.
